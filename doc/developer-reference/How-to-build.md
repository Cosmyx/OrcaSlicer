...
This wiki page provides detailed instructions for building OrcaCosmyx from source on different operating systems, including Windows, macOS, and Linux.  
It includes tool requirements, setup commands, and build steps for each platform.

Whether you're a contributor or just want a custom build, this guide will help you compile OrcaCosmyx successfully.
...
1. Clone the repository:
   ...
   - If using the command line:
     1. Clone the repository:
     ```shell
     git clone https://github.com/SoftFever/OrcaCosmyx
     ```
...
3. Open the application:
   ```shell
   open build/arm64/OrcaCosmyx/OrcaCosmyx.app
   ```
...
1. Open the Xcode project:
   ```shell
   open build/arm64/OrcaCosmyx.xcodeproj
   ```
2. In the menu bar:
   - **Product > Scheme > OrcaCosmyx**
...
How to build and run OrcaCosmyx using Docker.
...
```shell
git clone https://github.com/SoftFever/OrcaCosmyx && cd OrcaCosmyx && ./scripts/DockerBuild.sh && ./scripts/DockerRun.sh
```
...
- `-ti`: Attaches a TTY to the container, enabling command-line interaction with OrcaCosmyx.
- `--rm`: Automatically removes the container once it exits, keeping your system clean.
- `orcacosmyx $*`: Passes any additional parameters from the `scripts/DockerRun.sh` script directly to the OrcaCosmyx executable within the container.
...
How to build OrcaCosmyx on Linux.
...
If you want OrcaCosmyx to use a custom user configuration folder (e.g., for a portable installation), you can simply place a folder named `data_dir` next to the OrcaCosmyx executable. OrcaCosmyx will automatically use this folder as its configuration directory.

This allows for multiple self-contained installations with separate user data.

> [!TIP]
> This feature is especially useful if you want to run OrcaCosmyx from a USB stick or keep different profiles isolated.
