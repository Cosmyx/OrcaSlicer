...
namespace instance_check_internal
{
	...
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		...
		if (wndTextString.find(L"OrcaCosmyx") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	static bool  send_message(const std::string &message_text, const std::string &version)
	{
		...
		if ( !checker->IsAnotherRunning() ) */
		{
			...
			std::string		interface_name = "com.softfever3d.orca-cosmyx.InstanceCheck.Object" + version;
			std::string   	method_name = "AnotherInstance";
			//std::string		object_name = "/com/prusa3d/prusaslicer/InstanceCheck";
			std::string		object_name = "/com/softfever3d/OrcaCosmyx/InstanceCheck/Object" + version;
...
void OtherInstanceMessageHandler::handle_message(const std::string& message) 
{
	...
	boost::regex re(R"(^(orcacosmyx|prusaslicer|cura|bambustudio):\/\/open[\/]?\?file=)", boost::regbase::icase);
...
namespace MessageHandlerDBusInternal
{
	...
	static void respond_to_introspect(DBusConnection *connection, DBusMessage *request) 
	{
	    ...
	    const char  *introspection_data =
	        ...
	        "     </method>"
	        "   </interface>"
	        "   <interface name=\"com.softfever3d.orca-cosmyx.InstanceCheck\">"
	...
	//method AnotherInstance receives message from another OrcaCosmyx instance 
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.softfever3d.orca-cosmyx.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
	...
	std::string			 interface_name = "com.softfever3d.orca-cosmyx.InstanceCheck.Object" + instance_hash;
    std::string			 object_name 	= "/com/softfever3d/OrcaCosmyx/InstanceCheck/Object" + instance_hash;
	...
	if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
		BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another OrcaCosmyx instance is running.";
	    BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
	    dbus_connection_unref(conn);
	    return;
	}
