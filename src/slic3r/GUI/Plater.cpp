    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        bed_type_title->Bind(wxEVT_LEFT_UP, [bed_type_title, this](wxMouseEvent &e) {
            wxLaunchDefaultBrowser("https://github.com/Cosmyx/OrcaSlicer/wiki/bed-types");
        });
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#A200FF"));
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update Orca Cosmyx or restart Orca Cosmyx to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
                ...
                Semver             file_version;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by OrcaCosmyx, load geometry data only."), _L("Load 3mf"));
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Orca Cosmyx, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by OrcaCosmyx, load geometry data only."), _L("Load 3mf"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with OrcaCosmyx 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an OrcaCosmyx 2.3.1-alpha and uses "
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like OrcaCosmyx to automatically fix this by clearing the rotation template settings?"));
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - OrcaCosmyx");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Orca Cosmyx window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Orca Cosmyx failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    , m_action(2)
{
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/OrcaCosmyxTitle.ico") % resources_dir()).str();
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Orca Cosmyx(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
