...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(162, 0, 255));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(162, 0, 255)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(188, 79, 250), StateColor::Pressed), std::pair<wxColour, int>(wxColour(115, 79, 182), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(162, 0, 255), StateColor::Normal));
