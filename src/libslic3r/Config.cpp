...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or CosmoSlice header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; CosmoSlice";

        std::string orcacosmyx_gcode_header = std::string("; generated by ");
        orcacosmyx_gcode_header += SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(orcacosmyx_gcode_header.c_str(), line_c, strlen(orcacosmyx_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
