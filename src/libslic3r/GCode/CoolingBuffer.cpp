...
#undef EXTRUDER_CONFIG
    ...
    for (const CoolingLine *line : lines) {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL) {
            ...
            if (std::errc::invalid_argument != ret.ec) {
                // Append tool change command FIRST (before fan command)
                // This ensures the firmware switches to the new extruder before we set the fan speed
                new_gcode.append(line_start, line_end - line_start);

                if (new_extruder != m_current_extruder) {
                    // THEN update extruder and defer fan command
                    m_current_extruder = new_extruder;
                    change_extruder_set_fan(false);  // false = don't emit immediately, defer it
                    need_set_fan = true;  // Trigger deferred fan emission after tool change
                }
            } else {
                // Parse error - append line as-is
                new_gcode.append(line_start, line_end - line_start);
            }
