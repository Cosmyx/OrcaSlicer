...
# OrcaCosmyx Flatpak Build Script
# This script builds and packages OrcaCosmyx as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build OrcaCosmyx as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}OrcaCosmyx Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ ! -f "./scripts/flatpak/io.github.softfever.OrcaCosmyx.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/io.github.softfever.OrcaCosmyx.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="OrcaCosmyx-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
if ! flatpak-builder \
    "${BUILDER_ARGS[@]}" \
    "$BUILD_DIR/build-dir" \
    scripts/flatpak/io.github.softfever.OrcaCosmyx.yml; then
...
if ! flatpak build-bundle \
    "$BUILD_DIR/repo" \
    "$BUNDLE_NAME" \
    io.github.softfever.OrcaCosmyx \
    --arch="$ARCH"; then
...
echo ""
echo -e "${BLUE}To run OrcaCosmyx:${NC}"
echo -e "flatpak run io.github.softfever.OrcaCosmyx"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user io.github.softfever.OrcaCosmyx"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
