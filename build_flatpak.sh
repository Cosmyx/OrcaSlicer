...
# CosmoSlice Flatpak Build Script
# This script builds and packages CosmoSlice as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build CosmoSlice as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}CosmoSlice Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ ! -f "./scripts/flatpak/io.github.softfever.CosmoSlice.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/io.github.softfever.CosmoSlice.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="CosmoSlice-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
if ! flatpak-builder \
    "${BUILDER_ARGS[@]}" \
    "$BUILD_DIR/build-dir" \
    scripts/flatpak/io.github.softfever.CosmoSlice.yml; then
...
if ! flatpak build-bundle \
    "$BUILD_DIR/repo" \
    "$BUNDLE_NAME" \
    io.github.softfever.CosmoSlice \
    --arch="$ARCH"; then
...
echo ""
echo -e "${BLUE}To run CosmoSlice:${NC}"
echo -e "flatpak run io.github.softfever.CosmoSlice"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user io.github.softfever.CosmoSlice"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
