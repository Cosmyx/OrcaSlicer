# OrcaSlicer → CosmoSlice Rename Summary
...
### Source Code (4 files)
- `src/OrcaSlicer.cpp` → `src/CosmoSlice.cpp`
- `src/OrcaSlicer.hpp` → `src/CosmoSlice.hpp`
- `src/OrcaSlicer_app_msvc.cpp` → `src/CosmoSlice_app_msvc.cpp`
- `src/dev-utils/OrcaSlicer_profile_validator.cpp` → `src/dev-utils/CosmoSlice_profile_validator.cpp`

### Platform Files (4 files)
- `src/dev-utils/platform/unix/OrcaSlicer.desktop` → `CosmoSlice.desktop`
- `src/dev-utils/platform/msw/OrcaSlicer.rc.in` → `CosmoSlice.rc.in`
- `src/dev-utils/platform/msw/OrcaSlicer.manifest.in` → `CosmoSlice.manifest.in`
- `src/dev-utils/platform/msw/OrcaSlicer-gcodeviewer.rc.in` → `CosmoSlice-gcodeviewer.rc.in`

### Images/Icons (24 files)
All image files in `resources/images/` renamed:
- `OrcaSlicer*.png` → `CosmoSlice*.png`
- `OrcaSlicer*.svg` → `CosmoSlice*.svg`
- `OrcaSlicer*.ico` → `CosmoSlice*.ico`
- `OrcaSlicer*.icns` → `CosmoSlice*.icns`
- Including: mac icons, title images, gradient versions, grayscale variants

### Translation Files (21 files)
- `localization/i18n/OrcaSlicer.pot` → `CosmoSlice.pot`
- All 20 language `.po` files: `OrcaSlicer_*.po` → `CosmoSlice_*.po`
  - Languages: ca, cs, de, en, es, fr, hu, it, ja, ko, lt, nl, pl, pt_BR, ru, sv, tr, uk, zh_CN, zh_TW

### Distribution Files (3 files)
- `scripts/flatpak/io.github.softfever.OrcaSlicer.metainfo.xml` → `CosmoSlice.metainfo.xml`
- `scripts/flatpak/io.github.softfever.OrcaSlicer.yml` → `CosmoSlice.yml`
- `resources/profiles/Vivedino/OrcaSlicer-Troodon2-Bed-Texture.png` → `CosmoSlice-Troodon2-Bed-Texture.png`

### IDE Config (1 file)
- `.idea/OrcaSlicer.iml` → `CosmoSlice.iml`
...
### Core Identity
- ✅ `version.inc`: Updated `SLIC3R_APP_NAME` and `SLIC3R_APP_KEY` to "CosmoSlice"
...
- Window titles: "OrcaSlicer" → "CosmoSlice"
- Executable names: OrcaSlicer → CosmoSlice
- Bundle names (macOS): "OrcaSlicer" → "CosmoSlice"
- Desktop entries (Linux): Updated

### User Data Directories
Application will now use:
- **Windows:** `%APPDATA%/CosmoSlice/`
- **macOS:** `~/Library/Application Support/CosmoSlice/`
- **Linux:** `~/.config/CosmoSlice/`

### Build System
- CMake project names updated
- Install directories: `./OrcaSlicer` → `./CosmoSlice`
- Dependency directories: `OrcaSlicer_dep` → `CosmoSlice_dep`
...
2. Run `CosmoSlice.exe` (Windows) or equivalent
3. Verify window title shows "CosmoSlice"
4. Check that config files are created in new directory

### To Commit
```bash
git add -A
git commit -m "Rename OrcaSlicer to CosmoSlice
